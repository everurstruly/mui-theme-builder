...
export default function DesignResourceGeneratorButton() {
  const [open, setOpen] = React.useState(false);

  const displayDrawer = (shouldOpen: boolean) => () => {
    setOpen(shouldOpen);
  };
  ...
  return (
    <>
      ...
      <Drawer
        anchor="right"
        open={open}
        onClose={displayDrawer(false)}
        slotProps={{
          paper: {
            sx: {
              width: "var(--properties-panel-width, 25vw)",
            },
          },
        }}
      >
        <ResourceGeneratorDrawerContent />
      </Drawer>
    </>
  );
}
...
function ResourceGeneratorDrawerContent() {
