...
ThemeSheet is a complete MUI theme management system that:
...
```tsx
import { ThemePreviewPane, useThemeSheetEditValue } from './ThemeSheet';
...
function ColorPicker({ path }: { path: string }) {
  const { value, setValue, isControlledByFunction } = useThemeSheetEditValue(path);
...
```tsx
import { useThemeSheetHistory } from './ThemeSheet';

function HistoryControls() {
  const { undo, redo, canUndo, canRedo, historySize, isDirty } = useThemeSheetHistory();
...
```tsx
import { useThemeSheetStore, listBaseThemeIds } from './ThemeSheet';
...
```tsx
import { useThemeSheetStore, listComposables } from './ThemeSheet';
...
├── hooks/
│   └── useThemeSheetEditValue.hooks.ts    # Hook for UI controls
├── components/
...
```tsx
import { Composable } from './ThemeSheet/types';
...
useThemeSheetStore.setState(saved);
```
...
import { resolveThemeOptionsForPreview } from './ThemeSheet';
console.log(resolveThemeOptionsForPreview());

// Inspect store state
import { useThemeSheetStore } from './ThemeSheet';
console.log(useThemeSheetStore.getState());
```
